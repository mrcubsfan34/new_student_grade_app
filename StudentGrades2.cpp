...
double median(const vector<double>& v) {
  if (v.size() == 0)
    throw domain_error("cant compute median of
    empty vector");
  vector<double> v2(v);
...
istream& read_homework(istream& in, vector<double>&
homework) {
  if (in) {
    double x;
    homework.clear();
...
bool compare_names(const student_info& s1, const
student_info& s2) {
...
double opti_median_analysis(const vector<student_info>&
students) {
  vector<double> grades;
  transform(students.begin(). students.end(),
